 ...
 *
 * Plays well with a PHP Ratchet WebSocket server for example.
 * Very useful for Test-Driven Development with PHPUnit.
 *
 * @author Anonymous <people@earth.sun>
...
{
    // An error has occurred: Tried to write to closed or invalid stream.
    // Augment this if your server is slow to answer !
    public $timeout = 5;
...
//        echo $response['payload']."\n";
    ...
    public function connect($host, $port, $path, $origin = false)
    {
        ...
        $header .= "User-Agent: PHP Websocket Client"."\r\n";
        $header .= "\r\n"; // <= capital !

        $this->_socket = fsockopen($host, $port, $errno, $errstr, 2);
        socket_set_timeout($this->_socket, $this->timeout, 0) or die("fail : {$errno} {$errstr}");
        fwrite($this->_socket, $header) or die("fail : {$errno} {$errstr}");
        $response = fread($this->_socket, 2000); // 1500 ?

        if ('' == $response) throw new \Exception("Handshake's response is empty");
